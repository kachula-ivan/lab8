...
<body>
    <h1>Таблиця Tovar na prodaj i kupivlyu</h1>
    ...
    if($result = $mysqli->query('SELECT * FROM tovar_na_kupivlyu')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<h3>Звіт - Список товарів на покупку: </h3><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>Id</th><th>price</th><th>opys</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['id'], $row['price'], $row['opys']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    if($result = $mysqli->query('SELECT * FROM tovar_na_prodaj')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<h3>Звіт - Список товарів на продаж: </h3><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>Id</th><th>price</th><th>opys</th></tr>");   // <br> в html - розрив рядка
        /* Вибірка результатів запиту  */
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['id'], $row['price'], $row['opys']); //виводимо результат на сторінку
    ...
    <ul>
        <li><a href="showKorzyna.php">Корзина</a><br></li>
